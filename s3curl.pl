...
my $LOCALDOTFILE = $DOTFILENAME;
my $EXECDOTFILE = $EXECFILE . "/" . $DOTFILENAME;
my $HOMEDOTFILE = $ENV{HOME} . "/" . $DOTFILENAME;

my $DOTFILE = $HOMEDOTFILE;
if (-f $LOCALDOTFILE) {
  $DOTFILE = $LOCALDOTFILE;
} elsif (-f $EXECDOTFILE) {
  $DOTFILE = $EXECDOTFILE;
}
...
for (my $i=0; $i<@ARGV; $i++) {
    ...
    if ($arg =~ /https?:\/\/([^\/:?]+)(?::(\d+))?([^?]*)(?:\?(\S+))?/) {
        ...
        for my $attribute ("acl", "delete", "location", "logging", "notification",
            "partNumber", "policy", "cors", "requestPayment", "response-cache-control", 
